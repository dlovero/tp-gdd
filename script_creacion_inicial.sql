...
IF ((SELECT Cliente_Habilitado FROM DESCONOCIDOS4.CLIENTE WHERE Cliente_Id = @CLIENTE) = 1)
	BEGIN
		...
		FROM DESCONOCIDOS4.VIAJE 
		WHERE  Viaje_Cliente = 1  AND CONVERT(DATE,Viaje_Fecha_Hora_Fin) <= CONVERT(DATE,'2015-12-31') AND Viaje_Nro NOT IN (SELECT * FROM DESCONOCIDOS4.FN_VIAJES_FACTURADOS(@CLIENTE))
	END
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJES_A_FACTURAR','FN')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJES_FACTURADOS','FN')IS NOT NULL
...
BEGIN TRANSACTION	
	...
	FROM [DESCONOCIDOS4].FN_VIAJES_A_FACTURAR((SELECT I2.Cab_Fac_Cliente FROM INSERTED I2) , (SELECT I2.Cab_Fac_Fecha FROM INSERTED I2))
COMMIT
GO
