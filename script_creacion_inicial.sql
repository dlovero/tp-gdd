...
GO
 ...
 IF OBJECT_ID('[DESCONOCIDOS4].PRC_EXISTE_DNI','P') IS NOT NULL
	DROP PROCEDURE [DESCONOCIDOS4].PRC_EXISTE_DNI;
GO

--BUSCA SI YA EXISTE LA DNI, DEVUELVE 1 SI LO ENCUENTRA 0 POR EL CONTRARIO
CREATE PROCEDURE [DESCONOCIDOS4].PRC_EXISTE_DNI(@DNI NUMERIC(18))
AS
 BEGIN
 DECLARE @RESUL BIT
...
--EN LA FUNCION QUE CALCULA EL MONTO TOTAL A FACTURAR Y EN EL DETALLE DE LA RENDICION, DONDE MUESTRA EL MONTO DEL VIAJE
...
IF (@HABILITADO = 1 AND @TURNO IN (SELECT * FROM @TURNOS_CHOFER))--fin validacion
	BEGIN
		SELECT 
			...
			SUM(Viaje_Importe*0.3)	AS IMPORTE_A_RENDIR
		FROM DESCONOCIDOS4.VIAJE 
