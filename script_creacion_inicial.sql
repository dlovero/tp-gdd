...
AS
 BEGIN
 DECLARE @RESUL INT
 SET @RESUL =(SELECT TMarca_Modelo_id FROM [DESCONOCIDOS4].MARCA_MODELO WHERE CONCAT(TAuto_Marca,TAuto_Modelo)=CONCAT(@MARCA,@MODELO))
 RETURN @RESUL
END
GO
...
BEGIN 
	...
	FROM [DESCONOCIDOS4].AUTO A  LEFT JOIN [DESCONOCIDOS4].MARCA_MODELO  MM  ON A.Auto_Marca_Modelo=MM.TMarca_Modelo_id
	LEFT JOIN [DESCONOCIDOS4].MARCA MR ON MR.Marca_Id=MM.TAuto_Marca LEFT JOIN [DESCONOCIDOS4].MODELO MD ON MD.Modelo_Id=MM.TAuto_Modelo
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJES_FACTURADOS','FT')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].PRC_FACTURAR_A_CLIENTE','P')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJES_A_FACTURAR','FT')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].PRC_INSERTAR_FACTURA','P')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJES_RENDIDOS','FT')IS NOT NULL
...
--MONTO A RENDIR
IF OBJECT_ID('[DESCONOCIDOS4].FN_MONTO_A_RENDIR','FN')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].PRC_RENDICION','P')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].FN_VIAJES_A_RENDIR','FT')IS NOT NULL
...
IF OBJECT_ID('[DESCONOCIDOS4].PRC_INSERTAR_RENDICION','P')IS NOT NULL
...
/*
select * from DESCONOCIDOS4.ITEM_FACTURA
select * from DESCONOCIDOS4.FACTURA_REP


select * from DESCONOCIDOS4.USUARIO_ROL
select * from DESCONOCIDOS4.HOJA_MENU
select * from DESCONOCIDOS4.RAMA_MENU
